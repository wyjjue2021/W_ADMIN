...
//         hasSelectTotal, selectTotalFiledName,textColFieldName = 'operation'} = this.props
    ...
    onGridReady = (params) => {
     ...
     getAgGridColumnProps = (col) => {
        ...
        const {_pinnedColumns, cellArr,headerArr,pinnedCellArr} = this.state
        let __column = {
            key: col.fieldName,
            ...
            filter: 'customNumberFilter',                          // 用于子过滤器的浮动框架过滤器组件  
    ...
    render(){
        const { tableHeaderControlList,_pageSize,tableColumns } = this.state
        ...
        console.log(floatingFilterComponentFramework, 3)
        console.log(tableColumns.map(item => {
